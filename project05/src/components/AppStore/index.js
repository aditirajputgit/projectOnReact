...
function AppStore() {
  ...
  const getFilterId = () => {
    const filterApp = appsList.filter(user => user.category === activeTabId)
    return filterApp
  }

  const searchInputs = event => {
    const searchApp = event.target.value
    setSearchInput(searchApp)
  }

  const filterApp = getFilterId()

  const searchResult = filterApp.filter(app =>
    ...
    <>
      <div className="container">
        ...
        <ul className="container2">
          {tabsList.map(user => (
            <TabItem eachItem={user} upadted={updated} key={user.tabId} />
          ))}
        </ul>
        <hr />
        ...
        <ul className="container3">
          {searchResult.map(user => (
            <AppItem appList={user} key={user.appId} />
          ))}
        </ul>
      </div>
    </>
